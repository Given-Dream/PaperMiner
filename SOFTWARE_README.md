...
- “文章标题 / Markdown”使用 MinerU 首页结构、PDF 元数据、Markdown 标题和源文件名交叉核验，输出 `Title/文章标题.md` 与 `title_scan.json`；不调用 LLM，只有文件名或弱证据时标为“需核查”。
- “合并文章标题、同名章节、图表、代码/数据地址和参考文献”会生成 `文章标题_合并.md`，并继续按章节名称生成独立汇总文件，把每篇论文 `Word` 文件夹中的 Markdown 合并为 `图表汇总_合并.md`，把非空可用性报告合并为 `代码与数据可用性_合并.md`，把有效的逐篇参考文献报告按论文分组合并为 `参考文献_合并.md`；图表合并时会自动改写图片和表格的相对路径。
